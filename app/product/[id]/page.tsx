...
export default function ProductPage({ params }: PageProps) {
  ...
  const category = categories.find(c => c.slug === product.categoryId)
  const subcategory = subcategories.find(s => s.slug === product.subcategoryId)
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 pt-28 pb-4">
          <nav className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
            ...
            {category && (
              <>
                <ChevronRight className="w-3 h-3" />
                <Link href={`/shop/${category.slug}`} className="hover:text-foreground boty-transition">
                  {category.name}
                </Link>
              </>
            )}
            {subcategory && (
              <>
                <ChevronRight className="w-3 h-3" />
                <Link href={`/shop/${category?.slug}/${subcategory.slug}`} className="hover:text-foreground boty-transition">
                  {subcategory.name}
                </Link>
              </>
            )}
